...
def brash(proxy_dict):
    ...
    try:
        r = requests.get("https://blog.csdn.net/lwcwam/article/details/import requests
import multiprocessing
import time
import random
import logging
...
def brash(proxy_dict):
    ...
    try:
        r = requests.get("https://blog.csdn.net/lwcwam/article/details/144925766", headers=header, proxies=proxy_dict, timeout=10)
        r.raise_for_status()
    ...
    for _ in range(final):
        ...
        for _ in range(count):
            ...
            for result in results:
                result.get()
            pool.close()
            pool.join()", headers=header, proxies=proxy_dict, timeout=10)
        r.raise_for_status()
        logging.info("Request successful") # 成功日志
        print("successful")
    except requests.exceptions.RequestException as e:
